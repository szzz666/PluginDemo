...
public class MyChestMenu {
    public static void test1(Player player) {
        BigChestMenu menu = new BigChestMenu("test");
        menu.add(0, Item.get(Item.STONE_SWORD), () -> {
            menu.close(player);
            player.sendMessage("clicked 1");
        });
        menu.add(1, Item.get(Item.STONE_SWORD), () -> {
            menu.close(player);
            player.sendMessage("clicked 2");
        });
        menu.add(2, Item.get(Item.STONE_SWORD), () -> {
            menu.close(player);
            player.sendMessage("clicked 3");
        });
        menu.show(player);
    }
}
